...
export async function main(): Promise<void> {
  ...
  try {
    const portalPath = path.join(vaultDir, '_portal.md');
    const portalContent = [
      '# Myco Portal',
      '',
      `Open the Myco dashboard in your browser: [http://localhost:${server.port}/ui/](http://localhost:${server.port}/ui/)`,
      '',
      '> This file is generated by the Myco daemon on startup. Do not edit.',
    ].join('\n');
    fs.writeFileSync(portalPath, portalContent, 'utf-8');
  } catch { /* non-critical — vault may be read-only */ }

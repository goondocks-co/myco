...
/** Matches daemon.log and rotated files like daemon.1.log, daemon.2.log. */
const LOG_FILE_PATTERN = /^daemon(?:\.(\d+))?\.log$/;

/**
 * Query parsed log entries from JSONL log files on disk.
...
/** Discover and sort log files: oldest rotated first, current last. */
function discoverLogFiles(logDir: string): string[] {
  ...
  for (const file of files) {
    const m = LOG_FILE_PATTERN.exec(file);
    if (!m) continue;

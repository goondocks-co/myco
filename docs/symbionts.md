...
For OpenCode and Pi, `myco member provision opencode` or `myco member provision pi` writes Myco's plugin into the project itself (`.opencode/plugins/myco.ts`, `.pi/extensions/myco/index.ts`). The Myco plugin installed for all your projects steps aside in a project that has one, so each session is captured once, and your other projects keep the plugin they have. If the installed Myco plugin is from a version that does not step aside, or the project already has a plugin of your own at that path, provisioning stops before changing anything and says why. Pi loads a project's extension only after you trust the project in Pi (`/trust`). `myco member leave` removes the project's plugin again. OpenCode's Myco tools come from the project's own entry in `opencode.json`, which runs Myco locally and reaches your deployment with the project's membership. OpenCode combines a `myco` server in your user-level OpenCode config with the project's one, so if that user-level entry signs in somewhere else, carries its own environment, or is switched off, provisioning stops before changing anything and names the file. `myco member leave` removes the project's entry again, and leaves an entry Myco did not write for this membership as it is.

...
- **OpenCode's Myco tools are not moved:** its MCP stays the user-level entry, which on the dogfood host is the retained 1.x stdio server. Direct HTTP MCP for OpenCode is an open gap under #1244.

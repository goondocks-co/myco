...
- Provisioning refuses before any write while the installed global Myco plugin lacks `// myco:defers-to-member-plugin` (`MemberPluginConflictError`); a missing global plugin, or one Myco does not own, is no conflict.

...
**A Member Credential rotates; it never ages out** (#1417, decided 2026-09-26). Each token lives seven days and the member rotates it silently in the last quarter of that life, so a copied token dies within a week unless its holder keeps refreshing it. Theft is answered by **replay detection** (a superseded token presented again is recorded as `lineage_replayed`, and its lineage can be revoked) and by **owner revocation** from the dashboard — not by a fixed re-login age: the 90-day lineage ceiling is gone, and a machine in daily use never re-joins. Only **prolonged inactivity** does: a token that lapsed offline still rotates while its lineage refreshed within the last 90 days (`MEMBER_LINEAGE_IDLE_MS`, measured from the presented token's `issued_at`, the last successful refresh or the join), and a machine silent longer than that is refused with `lineage_expired` — the code every member build already treats as final — and shows the notice to ask an admin for an invite and run `myco login`.
...
| `blob_reservations` | KEEP | Core | Blk | Transient upload state: the key, size and expiry an ingest holds against a credential's byte quota until the bytes land or the reservation lapses. The reservation is also the upload's authority: its id is the generation the bytes are stored under, and registration, refusal, error and expiry each consume it exactly once | #898, #1316 |
...
| `member_credentials` | KEEP | Core | Blk | Every credential a member holds — hashed token, machine and runtime, lineage, expiry, bytes written, and the member that revoked it. A lineage has no age limit; a lapsed token rotates while the lineage refreshed within 90 days (§3.2) | #912, #1417 |

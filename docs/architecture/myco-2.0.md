...
| `myco_skills` | KEEP | MCP, Core | Blk | Answers from the skills that ship with the plugin, read from one generated catalogue rather than from storage, so a Deployment holding no rows still answers a fresh install's first call (**landed in #1156**). The generation pipeline, the candidate queue and their tables are deleted in #1170 (plan §2.4, D3) | C2 |

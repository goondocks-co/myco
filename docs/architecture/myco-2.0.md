...
3. **The queue** (#1091 S2): a queued run is a run row — `agent_runs.status = 'queued'` with `queued_at`, `held_by` (the limit that holds it) and `dispatch_spec` (the launch it was asked for), no credential until it launches — so the Agent runs page, the run reads and retention all see it as a run. Limits are Settings leaves — `agent.limits.concurrent_runs`, `agent.limits.task_concurrent_runs`, `agent.limits.task_runs_per_hour`, `agent.limits.fleet` — unset meaning unbounded. `admitDispatch` in `core/harness.ts` decides between `prepareDispatch` and `launchDispatch`; a dispatch past a limit is queued and wakes the Deployment, never refused. `drainQueue` runs on the wake tick and after every terminal write the update route lands, oldest first, preparing and admitting each row again as the load stands; a queued run keeps the Deployment awake. The fleet size is an operator setting written by `myco server config` (S4), counted like any limit.
...
| `agent.limits.fleet` | NEW | Deployment | Core | How many runtimes the Deployment can start at once; written by the operator verb, counted like a limit | #1091 |

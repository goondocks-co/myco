...
| Deploy against the runs in flight | REPLACE | W | Blk | New in 2.0: `myco server update --target cloudflare` waits for the tasks running on the Deployment before it pushes (bounded by each run's own budget plus the overrun margin; `--no-drain` ships without waiting), then watches the container instances reach the new version before it returns and records where they landed. A deploy shipping the image already running rolls nothing. | #1115 |

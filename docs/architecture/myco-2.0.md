...
| `cortex.canopy.refresh.background_enabled` | REPLACE | Deployment | Core | Deployment-side map refresh policy: switches the clock's `canopy-map` schedule on. | #1378 |
| `cortex.canopy.refresh.background_period_minutes` | REPLACE | Deployment | Core | Deployment-side map refresh schedule: the interval of the clock's `canopy-map` schedule. | #1378 |

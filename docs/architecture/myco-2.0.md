...
| `titling-backfill` | KEEP | Core | Blk | New in 2.0 (#1203): titles the sessions an import brought, which arrive untitled by design — newest first, `TITLING_BACKFILL_BATCH` per wake, only once every transcript of the session is parsed, through the same `claim` dispatch a session's own end makes (one attempt, opening prompts, never over a standing title), under the dispatch limits. Its bounds are the task schedule vocabulary every other clock-driven task uses: a declared `title-summary` block (`TITLING_BACKFILL_SCHEDULE`, **`enabled: false`** so nothing historical is processed merely on deployment, `maxRunsPerDay` 24, `intervalSeconds` 900) under the owner's `agent.tasks` override, behind `agent.scheduled_tasks_enabled`; the ceiling is counted across the Deployment by its actor, `backfill`. A person starts and stops it on Operations (`/api/titling-backfill`, which writes that override through the one settings writer) or in the override document; a person's ask from a session's page is never counted against it. It keeps no state of its own beyond the claim stamp and the run rows run retention already prunes | #1203 |
...
| `agent.tasks` | REPLACE | Deployment | Core | Per-task overrides the Deployment applies to its own harness runs; the `title-summary` block's `enabled`, `maxRunsPerDay` and `intervalSeconds` bound the imported-session backfill (#1203) | #919 |

...
**Rollout order: the Deployment before its members.** Capture carries the session's commit as `headSha` on `session.start` and `session.end`, and `dirty` on `session.end`. A Deployment older than schema v46 refuses those payloads as `unknown_field`, so a Deployment is upgraded before the members that capture into it.

...
| `blobs` | KEEP | Core, W, C | Blk | Every stored blob's key, size, media type, writing credential and generation; the bytes sit in R2 on W and on the volume on C under `<project>/<key>~<generation>` (`<project>/<key>` for a row from before schema 42), a name no other write uses | #898, #1316 |

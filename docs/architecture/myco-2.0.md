...
| Offline capture | KEEP | M | Blk | The write-ahead spool stays, drained on the next hook with backoff; after a drain that delivered everything the hook prunes the state of sessions delivered long ago, and an unacknowledged spool is quarantined rather than deleted (plan §2.2, §7; **landed in #1155**). A refusal is classified by what it judges (`REFUSAL_SUBJECT`, `member/constants.ts`): one that judges the record or its session drops that record, logged; one about the Deployment, the credential, the clock or the server's version holds the record and the rest of its session's events, in order, behind a per-session wait a backlog walk honours and `myco member drain` does not; an unclassified one is final for its record after 72 h at the longest wait | #1155, #1389 |

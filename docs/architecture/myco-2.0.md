...
| `title-summary` | KEEP | Core | Blk | The **session titles and summaries** outcome: dispatched on a session's end and on an ask, run on a worker, no direct provider call on the server (plan §2.5). Its close evidence is a report with action `summary` and — the run's whole product — a title on the session its dispatch named, read off the run's own recorded context (`core/run-postconditions.ts`). A harness that ends its turn without ever calling the Deployment leaves neither, and the run is recorded `failed` naming what it owed rather than `completed` on the worker's word | B2 |
...
| `agent_run_events` | KEEP | MIGRATE | Core | Blk | Also the Deployment's own record of what a run called: every tool call made under a run credential and answered lands one `run_tool` row naming the tool and the op (`mcp/server.ts`, `mcp/context.ts`), read back on the run's detail page. A refused call writes nothing at all and is named in telemetry instead, so a credential cannot turn calls it may not make into rows. A run's turns happen inside a harness the Deployment cannot see, so these calls are the only part of a run it observes directly — and an empty list against a closed run is how a harness that never called back is read | #919 |

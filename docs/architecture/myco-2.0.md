...
| Harness | The Deployment runs no coding harness; an attached worker serves the three prompted outcomes. Its embedding-only runtime uses the authenticated run routes in bounded response work; the wake invocation awaits the work it starts | Workers attach the same way, over the Deployment's own HTTP surface under an administrator's credential, so a worker inside the laptop process and a worker on a machine of its own run identical code. `myco server run` starts one in-process unless `--no-worker`. A dispatch is never refused for want of a harness: it queues, and an operator reads the wait on the run. Three tasks keep the push-launch seam until the sweep — `embedding-reconcile` and `canopy-map`, whose surface is a server-side step loop rather than MCP, and `container-smoke`, the containerized runtime's own proof. Native `embedding-reconcile` binds an in-process runner over the same authenticated run routes and bounded step loop; its task allowlist admits no container-only task, and stopping the Deployment drains this runner before releasing its volume |

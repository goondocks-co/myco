...
| Harness | The Deployment runs no coding harness; an attached worker serves the four prompted outcomes. Its embedding-only runtime uses the authenticated run routes in bounded response work; the wake invocation awaits the work it starts | Workers attach the same way, over the Deployment's own HTTP surface under an administrator's credential, so a worker inside the laptop process and a worker on a machine of its own run identical code. `myco server run` starts one in-process unless `--no-worker`. A dispatch is never refused for want of a harness: it queues, and an operator reads the wait on the run. Two tasks keep the push-launch seam until the sweep — `embedding-reconcile`, whose surface is a server-side step loop rather than MCP, and `container-smoke`, the containerized runtime's own proof. Native `embedding-reconcile` binds an in-process runner over the same authenticated run routes and bounded step loop; its task allowlist admits no container-only task, and stopping the Deployment drains this runner before releasing its volume |
...
The machine-side product contains the `myco` binary, hooks, spool, local registry/configuration, and worker mode. It owns **no vault, no dashboard and no resident service other than its worker**: reconciliation is on demand through `myco update` and `myco doctor`, run by the setup skill and the installer (plan §2.5). The worker runs as a login service (`myco worker install`, `runner/service.ts` over the per-user service in `server/service.ts`): a LaunchAgent on macOS, a systemd user unit on Linux, a logon task on Windows, one per Deployment per member home, running `myco worker --server <url>` under that home's `MYCO_HOME` with its output in `<MYCO_HOME>/logs/`. `myco member join` installs it where the membership, a logged-in harness and the installed binary are all present, the Deployment is not this machine's own native one, and the Deployment admits the credential as a worker (asked through `POST /worker/lease` naming no run, which the same administrator gate answers and which changes nothing). A worker the Deployment refuses terminally — `not_admin`, `unauthorized`, `no_membership` — records the refusal under `<MYCO_HOME>/worker/refusals/` and exits successfully, so the service does not restart it into the same answer, and a later install does not write over a recorded refusal unless forced. A worker whose program on disk is replaced exits non-zero so the service starts the new one. `member leave` of the last project on a Deployment removes the unit, `myco remove` removes every unit the home installed and every unit whose membership is gone, and `member status` and `myco doctor` report it. Capture stays hook-invoked and write-ahead per the member seam (#901).
...
- **The installer** (POSIX `sh`, PowerShell) places the binary and writes capture hooks with absolute paths. It adds capture, plan capture, import and the worker, the last as a login service that restarts on failure.
...
Member provisioning reconciles hooks and the MCP entry independently. An unchanged hook file does not suppress a missing or changed MCP entry, and the join command reports an unchanged registration as such. `myco member join --provision <agent>` and `myco member provision <agent>` share one provisioning step (`provisionAgent`, `cli/member.ts`); `provision` runs it from the membership already recorded for the project; no token is supplied or changed.

**Member provisioning is global by default.** The selected project supplies the recorded membership; it does not select a project-local installation. Hooks, plugins and MCP entries use the agent's user-wide targets. Codex uses `~/.codex/hooks.json` and `~/.codex/config.toml`; Claude Code uses `~/.claude/settings.json` for hooks and `~/.claude.json` for MCP. After writing the global replacement, provisioning removes this project's member hook and MCP entries through `SymbiontInstaller`, preserving unrelated configuration. Leaving a project preserves the global integrations used by other projects.

An authenticated member can initialize MCP and list tools without a Project header. Every tool call on that connection names an existing Project through `project`; no default Project is inferred and discovery creates no Project. A connection with a valid Project header retains its default for reads. Writes always name their Project. Run and grant credentials retain their own project restrictions.

Provisioning refuses to replace global hooks owned by a different installation or an MCP entry for a different Deployment. This includes the retained 1.4 installation: keeping its local capture working takes precedence over removing dogfood overrides. The project-scoped mechanics described below remain for existing installations and their cleanup; the member CLI no longer creates them. A global MCP headers helper started outside a bound project resolves the explicitly named Deployment and sends no project header. Inside a bound project, its Deployment must still match.

**The member MCP entry is the Deployment's remote HTTP `/mcp` wherever the host can refresh a rotating credential** (#1237). A manifest declaring `memberMcpHeadersHelperKey` gets `url = <registry serverUrl>/mcp` plus `myco member mcp-headers --credential registry --server <serverUrl>`, with no token in the file. Codex uses `http_headers_helper` in `~/.codex/config.toml`; Claude Code uses `headersHelper` in `~/.claude.json`, its documented [user-scope MCP file](https://code.claude.com/docs/en/mcp#mcp-installation-scopes). The helper reads the registry each time and never rotates credentials. It verifies that the credential belongs to the configured Deployment. Project overrides retained for dogfooding still undergo the host-specific merge-conflict checks before any write. **Cursor keeps the stdio member bridge, an explicitly unresolved limitation:** its remote entries take static headers or OAuth and ignore a header helper. Static headers cannot follow a rotating member bearer, and the Deployment serves no MCP OAuth, so Cursor has no compatible refresh path (#1337).

**OpenCode and Pi member plugins install globally.** Their targets are `~/.config/opencode/plugins/myco.ts` and `~/.pi/agent/extensions/myco/index.ts`. Both hosts also load project plugins, so an existing project member plugin is removed after the global replacement is written. Existing dogfood project overrides use `memberHooksTarget` (`.opencode/plugins/myco.ts`, `.pi/extensions/myco/index.ts`), carrying the line `// myco:member-plugin`; their retained compatibility contract is:
- OpenCode's 1.4 plugin returns no hooks when any `.opencode/plugins/myco.ts` from `ctx.directory` up to `ctx.worktree` carries that line.
- Pi's member extension, as the last statement of its factory and only when its binary resolves as runnable, sets `Symbol.for("myco.member-extension")`, and the 1.4 factory returns when it is set.
- Provisioning refuses before any write while the installed global Myco plugin lacks `// myco:defers-to-member-plugin` (`MemberPluginConflictError`); a missing global plugin, or one Myco does not own, is no conflict. It also refuses, and leaves the file as it is, when the project's plugin path already holds a file Myco does not own. Both checks run in the one private writer (`writeMemberPlugin`) before its single write.
- The member plugins follow each host's callback contract. Pi calls `handler(event, ctx)`: the transcript is `ctx.sessionManager.getSessionFile()`, the prompt is `event.prompt`, and tools run as `execute(toolCallId, params)`, answering `{ content }` and throwing on failure. OpenCode's `chat.message` carries only the user message, so the assistant's text is taken from `experimental.text.complete`.
- `member leave` removes the member plugin, with or without `--purge`, only while it carries the member line, so the host falls back to its global plugin.
- **OpenCode's Myco tools use the global MCP registration.** `member provision opencode` writes `~/.config/opencode/opencode.json` `mcp.myco` as the stdio bridge `<member-home binary> mcp --credential registry`, the same shape Cursor keeps: OpenCode 1.18.21 has no header-helper field, static headers cannot follow a rotating member token, and the Deployment serves no MCP OAuth, so direct HTTP stays out of reach for this host (#1337 tracks the same limit for Cursor).
- OpenCode merges a project's `myco` server into the user-global one of the same name rather than replacing it: the project's `command` wins, while the global entry's `environment`, `enabled`, `url` and `headers` survive into it (`opencode debug config`, 1.18.21). `memberMcpGlobalMerge` marks the hosts that do this — Codex and OpenCode — and provisioning refuses before any write when the surviving keys would point the member's server elsewhere, give it another credential, or switch it off.
- `member leave` removes the member's MCP entry as well, with or without `--purge`, and only the entry that carries the member credential, so a 1.4 project install's server at the same name is left to the install that owns it.
...
**Four run outcomes and one config leaf** (plan §2.5; the map by #1378):
...
| The repository map — a directory skeleton and golden-path files, grounded in file digests | `canopy-map`, `canopy-background-scan` | yes; writes the Project's one map through `myco_run_map` |
...
|---|---|---|---|---|---|
| `member` | KEEP | M | Blk | Already the 2.0 surface; gains Deployment-aware join, `provision` (an agent's hooks and MCP entry from the recorded membership; no token supplied or changed) and `mcp-headers` (the server-bound headers helper a remote MCP entry names, §3.4) | #916, #1237 |
...
| `mcp` | KEEP | M, MCP | Blk | Retargeted at the Deployment as remote HTTP MCP; tenancy travels as the tools' `project` parameter (plan §3 D1). Codex and Claude Code members reach `/mcp` directly through a headers helper; the stdio bridge remains for Cursor, whose remote entries cannot refresh a rotating member token (§3.4, #1337) | A6, #1237 |
...
| `doctor` | REPLACE | M | Blk | Checks member wiring, credential liveness, spool depth, harness detection **and Deployment reachability from this machine**; the second on-demand reconcile verb (plan §2.5). A joined project runs the member's checks (`cli/member-doctor.ts`): membership, `/health` and an authenticated tool list, credential and renewal, spool and refusals, member capture per harness, the member MCP entry and the worker service. A root with no membership keeps the 1.4 checks | C2 |
...
| `stats` | REPLACE | M, Core | Blk | Deployment storage/schema health. A joined project reports its sessions, activity and recent runs from `myco_cortex` `projects_activity` and `myco_agent` (`cli/member-reads.ts`), and the Deployment's health over the member credential (`POST /members/status`, Deployment-wide and resolving no Project): the target, schema expected and found, the bytes this machine's credential stored — information, never a limit (#1416) — the transcript retention window ("raw bytes kept forever (no retention set)" or "kept N days after processing"), the bytes recorded blobs hold, and the database measurements store maintenance last recorded, each fact the target cannot report named unavailable with its reason. A root with no membership keeps the 1.4 handler, which reads a local SQLite file | #922 |
| `logs` | REPLACE | M | Blk | Local log files under `MYCO_HOME` with a CLI view; server logs are a separate surface (**UI**). A joined project shows the worker service's logs and the spool's refusal log (`cli/member-logs.ts`); a root with no membership keeps the 1.4 daemon log | #922 |
| `config` | REPLACE | M, Core | Blk | Two tiers: Member Settings local, Deployment Settings server-side. A joined project reads Deployment Settings over the member credential (`POST /members/settings`, Deployment-wide and resolving no Project; every URL value leaves without its userinfo, query and fragment, and provider credentials stay in the secret store; `cli/member-config.ts`), with writes kept to the dashboard; Member Settings have no owner the member reads yet (#1393), so `get` says so and `set` refuses. A root with no membership keeps the 1.4 tiers | #915 |
...
| `myco_cortex` | REPLACE | MCP, Core | Blk | Serves `instructions`, its new default op, answering the `instructions.template` Settings leaf and the Project id beside it, and `canopy_map`, the Project's repository map rendered as markdown with the commit it was read from (`read/canopy.ts`, #1378). `notifications` (#922), `maintenance_summary` (#923), `digest` and `canopy_entry` stay declared on the tool and answer `not_served`, so a 1.4 runtime keeps its own handlers while a Deployment offers none; `digest` goes with its artifact in #1170, its loss recorded (plan §1, §3 D2), and a Deployment never offers `canopy_entry`, whose per-file entries stay dropped. Blocked MCP levers: `instructions` rides the `initialize` result, but `server/discover` stays unregistered — a client hearing a discover answer skips `initialize` and envelopes its requests, which the per-request JSON transport does not speak — and `cacheHints` is emitted only by that same codec, so a `tools/list` hint would be discarded | #1150 |
...
| `embedding-reconcile` | KEEP | Core | Blk | Not a run outcome: a deterministic server job on the wake tick, admitted by the embedding provider (plan §2.5). Cloudflare uses Workers AI `@cf/baai/bge-m3` and Vectorize, with bounded runs owned by the request deferral or awaited wake invocation; self-hosted uses sqlite-vec with configured Ollama, OpenAI-compatible, OpenAI or OpenRouter embeddings. Run steps reconcile missing and stale vectors, orphan deletion and spore hubness. | #1124 |
| `canopy-map` | KEEP | Core | Blk | The fourth run outcome (#1378, reversing plan §1's drop; content contract #910/#920): one prompt over a checkout, like `vault-seed`. The Deployment builds the prompt and the standing rules (`core/canopy-input.ts`); the harness explores `repo/` with its own file, search and Git tools and grounds every annotation in `{path, sha256}` pairs read from the worker's `repo.sha256` listing, so a later pass revisits only the domains whose grounding changed. The worker's repository step pins the commit and the map input together — the hash of the commit under the Deployment's exclusions, and the revision the run succeeds — and the run reads its prior map and writes the new one through the run-only `myco_run_map` (`get`, `write`). Its close evidence is a report with action `canopy_map` or `canopy_map_unchanged` and the current map row pinned to the source and repository this run was dispatched against, with `source_run_id` naming this run or the revision it succeeded (`canopyMapWrittenBy`); a second pass over the same commit closes unchanged. Budgeted 900 s; admitted by `canopy`; scheduled every six hours when an owner switches it on, and only after work was captured since the last completed map run started, or since the stored map where no run completed (`has-capture-since-map`). | #1378 |
...
| `canopy-describe` | DROP | — | Blk | Per-file descriptions and entry embeddings stay dropped; the map is kept without them (#1378) | #1170 |
...
Automatic schedules count their own runs over a trailing 24-hour window. Ad-hoc owner requests bypass this ceiling and do not consume automatic slots or reset the automatic interval. Both paths retain the shared concurrency and execution limits; a request waiting for execution capacity queues normally. The dispatch records its actor on direct and queued runs, and replacements retain that attribution. Older runs with no usable actor count conservatively against the automatic budget, including their replacements.

Extraction allows 12 automatic runs per trailing day. Three slots are reserved for live sessions that ended within the trailing 24 hours, have unread prompts, and have fully parsed transcript material. History-only work stops after nine automatic runs. The reservation uses the same primary-session selector as extraction: completed live capture takes priority over imports, then the newest eligible session wins. At least one quarter of each multi-prompt page remains available to other sessions, so a recent-session run can also advance history. `agent.tasks` schedule overrides can change `reservedRunsPerDay.count`; zero disables the reservation. Ad-hoc extraction remains available at either automatic threshold.

A refused automatic run is not queued. The next wake checks the trailing window again. The clock records `max_runs_per_day` or `reserved_runs_per_day` once per refusal episode, keyed by project, task, filling instant and refusal reason. Skipped rows consume no slots. An episode spanning midnight retains its row; a replacement can move the filling instant and create another refusal row without increasing the count. A zero ceiling with no prior entry uses one stable refusal row. Tests cover these decisions in `tests/myco-server/scheduled-tasks.test.ts`, owner requests in `harness-dispatch.test.ts`, replacement accounting in `replaced-runs.test.ts`, and the scheduler on both deployment targets in the `scheduled-tasks` parity scenario.
...
| `transcript-retention` | KEEP | Core | Blk | New in 2.0: prunes raw segments behind the parse cursor past the Deployment's `retention.transcripts` window and frees the blobs no surviving row references — a reference being any entry in the one catalogue every cleanup reads (`packages/myco-server/src/core/blob-references.ts`: every projected blob column, the tool call's spilled input and output among them, and the raw event log's key for compaction kinds); derived rows and the transcript record are never pruned | A3 |
| `object-release-drain` | KEEP | Core | Blk | New in 2.0: the only store deleter, run by the Deployment's own clock and never by an owner's wake. It consumes expired upload authorities, deletes journaled objects and removes each journal row only once the store acknowledged the delete, and with no recovery hold open decides recorded release candidates, backups against the retention policy then in force | #1316 |
| `recovery-hold-release` | KEEP | Core | Blk | New in 2.0: releases the open recovery hold only when the producer answers that its attempt advances no further or that no attempt carries it; an attempt still advancing, or a producer that cannot answer, keeps the hold; no interval releases it | #1316 |
| `titling-backfill` | KEEP | Core | Blk | New in 2.0 (#1203, #1381): the titling convergence — every ended, untitled session with parsed inline material is claimed, newest first, `TITLING_BACKFILL_BATCH` per wake, through the same `claim` titling a session's own end makes (opening prompts, never over a standing title), under the dispatch limits. A session its own live capture owes a title converges without the backfill switch: an end request whose attempt ended untitled — a run that failed, or one no worker took before the queue expired it — and a session captured live (not wholly imported) that ended with no request. A wholly imported session, which arrives untitled by design, waits for the backfill switch. A live end request's first attempt stays `session-titling`'s. A retry waits for the run window to pass and for no title run of the session to be queued or running, and the claim refuses a session once workers have taken `TITLING_MAX_ATTEMPTS` (3) attempts on it: `sessions.titling_attempts` counts one logical `claim`-mode run at its first worker claim (a trigger on `agent_runs`, marked by `attempt_counted`), so a run nobody took costs nothing, a lease that lapses and is claimed again counts once, a successor of a counted run adds nothing, and an owner's ask is neither counted nor bounded. A retry the ceiling refused keeps the stamp it replaced, so it stays a retry. What live capture owes is not gated by `agent.scheduled_tasks_enabled`: it completes capture-driven titling as `session-titling`'s first attempt does, bounded by the block's ceiling, pace and states and by the attempt limit. `idx_sessions_untitled_ended` is its ordered path. The session page names why an ended session is untitled. Its bounds are the task schedule vocabulary every other clock-driven task uses: a declared `title-summary` block (`TITLING_BACKFILL_SCHEDULE`, `maxRunsPerDay` 24, `intervalSeconds` 900, `runIn` active and idle) under the owner's `agent.tasks` override, counted across the Deployment by its actor, `backfill`; the block's **`enabled: false`**, behind `agent.scheduled_tasks_enabled`, admits wholly imported sessions only, so no imported history is processed merely on deployment. A person starts and stops the imported backfill on Operations (`/api/titling-backfill`, which writes that override through the one settings writer) or in the override document; a person's ask from a session's page is never counted against it | #1203 |
| `session-maintenance` | DROP | — | — | No job: a session's lifecycle is event-driven at the `session.end` and `prompt` projections (`ingest/projections.ts`). An agent's end hook, an import, or a person's **End session** on the session page (`POST …/sessions/{sessionId}/end`, one server-origin `session.end` through ingest) ends it, an end older than the newest live user turn is left unapplied, and a newer live user turn reopens it; nothing ends a session by age (#1277) | #1277 |
...
| `database-integrity-check` | REPLACE | Core, W, C | Blk | C runs SQLite's full `integrity_check` and `foreign_key_check` on a read-only thread beside the serving connection, held exclusive by the serving process for as long as it runs; the run continues past the clock's tick or the owner's request that claimed it, an owner's `POST /api/maintenance/integrity/run` is answered with the running claim, and the serving process drains the run before it closes the store. W runs `quick_check` and `foreign_key_check`, the checks D1 documents (https://developers.cloudflare.com/d1/sql-api/sql-statements/); D1 refuses `integrity_check` and `page_count`. D1's 30-second query limit (https://developers.cloudflare.com/d1/platform/limits/) bounds a hosted run. W reports its size from `meta.size_after` (https://developers.cloudflare.com/d1/worker-api/return-object/); the plan's size limit and the daily row quota are not readable by a Worker and are reported unavailable, and a query D1 refuses at a limit (https://developers.cloudflare.com/d1/observability/debug-d1/) is named `store_quota` or `store_size` | #1276 |
| `canopy-background-scan` | REPLACE | Core | Blk | Replaced by the clock's own `canopy-map` schedule (`TASK_SCHEDULE`, `core/jobs.ts`): off by default, switched on by `cortex.canopy.refresh.background_enabled`, spaced by `cortex.canopy.refresh.background_period_minutes`, and held by the `has-capture-since-map` precondition. No scan runs Deployment-side; the worker's checkout is the scan. | #1378 |
| `release-provenance-reconcile` | REPLACE | Core | Blk | A tick job (`core/release-provenance.ts`): checks each enabled Project at the Deployment's interval or on an owner's request, within the Project's GitHub lookup budget, through GitHub's API rather than a checkout; one check holds a Project at a time and publishes only under the settings it read | #1274 |
...
| `canopy_maps` | KEEP | REBUILD | Core | Blk | Derived; rebuilt by the map task. One row per Project, written only by a held map run against the revision it succeeded, read by `myco_cortex` `canopy_map` and the Project home. | #1378 |
...
| `blob_reservations` | KEEP | Core | Blk | Transient upload state: the key, size and expiry an upload holds until the bytes land or the reservation lapses; it holds no room against any bound (#1416). The reservation is also the upload's authority: its id is the generation the bytes are stored under, and registration, refusal, error and expiry each consume it exactly once | #898, #1316 |
| `blobs` | KEEP | Core, W, C | Blk | Every stored blob's key, size, media type, writing credential and generation; the bytes sit in R2 on W and on the volume on C under `<project>/<key>~<generation>` (`<project>/<key>` for a row from before schema 42), a name no other write uses. From schema 42 a row cannot be inserted without a generation, and a row with one cannot be deleted unless its exact object is journaled; rows from before step 42 are not fenced | #898, #1316 |
| `object_releases` | KEEP | Core, W, C | Blk | The journal of stored objects nothing registers, by physical key, kept until a store delete is acknowledged; the clock-owned drain is its only consumer; excluded from portable backups and cleared on restore | #1316 |
| `blob_release_candidates` | KEEP | Core, W, C | Blk | Blobs a deletion may have freed, recorded in the transaction that removed the rows naming them and judged again with no recovery hold open; excluded from portable backups | #1316 |
| `backup_release_candidates` | KEEP | Core, W, C | Blk | Catalogued backups retention let go of while a recovery hold was open, judged again against the policy in force; excluded from portable backups and cleared on restore | #1316 |
| `recovery_holds` | KEEP | Core, W | Blk | The hold a hosted recovery export opens before admission, released only on the producer's authoritative answer; at most one open; excluded from portable backups and cleared on restore | #1316 |
| `restore_reference_guard` | KEEP | Core, W, C | Blk | Never holds a row: an additive restore inserts into it to abort a batch whose rows would name a blob the Deployment does not register | #1316 |
...
| `member_credentials` | KEEP | Core | Blk | Every credential a member holds — hashed token, machine and runtime, lineage, expiry, bytes written, and the member that revoked it. `bytes_written` is a reporting counter carried across rotation and compared by no admission; schema step 48 dropped its lifetime CHECK (#1416) | #912, #1416 |
...
| `project_release_provenance` | KEEP | Core | Blk | One Project's release provenance settings — enabled, GitHub repository, production and integration refs, package map, unknown reporting, lookup budget — the sealed slot of a credential used only for release lookups, the running check's claim, and the latest check's outcome | #1274 |
...
| Session deletion | KEEP | Core, UI | Blk | New in 2.0: a tombstone suppresses a session, clears its title and summary, and removes captured projections while retaining session identity, saved knowledge and other sessions — reads through one predicate at every seam, writes through a check shared by every kind, so live capture cannot repopulate what a person deleted. The tombstone outlives the rows and gates re-import (plan §2.2) | A3, A4 |
...
| Deploy against the runs in flight | REPLACE | W | Blk | Prompted outcomes execute on attached workers that a Deployment deploy never replaces. Hosted embedding runs are bounded work within a request or wake invocation; a run interrupted before closure is failed by the existing bound sweep and its persisted journal is reconciled on a later wake. The container drain and the rollout watch retired with the container. The self-hosted target keeps its own wait, bounded by the harness stop grace | #1115 |
...
| Offline capture | KEEP | M | Blk | The write-ahead spool stays, drained on the next hook with backoff; after a drain that delivered everything the hook prunes the state of sessions delivered long ago, and an unacknowledged spool is quarantined rather than deleted (plan §2.2, §7; **landed in #1155**). A refusal is classified by what it judges (`REFUSAL_SUBJECT`, `member/constants.ts`): one that judges the record or its session drops that record, logged — the Deployment answers a field outside its declared shape or bound with `invalid_field`; one about the Deployment, the credential, the clock or the server's version holds the record and the rest of its session's events, in order, behind a per-session wait a backlog walk honours and `myco member drain` does not, and is logged once, marked held; so is a record whose staged bytes are on disk but unreadable; an unclassified one is final for its record after 72 h at the longest wait. A probing hook whose own session is held still delivers the other sessions' backlog | #1155, #1389 |
...
| Capture admission | KEEP | M, Core | Blk | Capture is Myco's foundation and is never refused for volume (#1416): a member's capture — events, transcript segments, blobs, attachments, imports — is admitted while its credential is live (`credentialLive`), and `member_credentials.bytes_written` is information only. Abuse is bounded by guards that never block ordinary work: the per-request body cap (`MAX_BODY_BYTES`) and blob cap (`MAX_BLOB_BYTES`, 25 MiB; the member ships 8 MiB slices), the source and token rate limits, and revocation. Storage is managed by the owner's `retention.transcripts` window over raw transcript bytes the parse has processed, default keep-forever. The bounded import's window and per-agent cap stay: backfill policy, repeatable on request, never met by live capture. `project_archived` stays an operator decision and `lineage_expired` the credential lifetime. A member parks on `quota` only from a server built before #1416 and drains once the Deployment is updated; `tests/meta/capture-never-refused-for-volume.test.ts` holds that no admission compares the counter | #1416 |
...
- **Eval suite** (**#1154**) — recorded real sessions as fixtures with a redaction gate before commit, a hand-annotated gold set of 30–50 cases, deterministic graders per PR against replayed recordings, and a weekly capped judged run required on releases (plan §2.5, §7). The fixture gate is installed with `npm run hooks:install` during contributor setup. It checks Git's staged blobs using the same redaction rules as the CI fixture sweep; the gold set, judged runs, and KPI feed remain pending.
...
**Capability master gates are fail-closed and that is a mechanism, not a default.** One of 1.4's four gates goes with its capability — `skills.enabled` with the generation pipeline — leaving `vault_evolution.enabled`, `cortex.enabled` and `cortex.canopy.enabled`, which admits the map run. Today all four default **`true`** in the schema, and `capabilityEnabled` returns `defaultEnabled ?? true` for an absent path (`config/capabilities.ts`). What actually makes a new project capture-only is a *write at provision time* — `reseedCaptureOnly()` seeding `false` for every gate (`vault/provision.ts`). On a Deployment, where Projects appear from a member's first write with no ceremony, the server-side predicate must therefore be the **inverse**: an absent row reads **disabled**. Otherwise every new Project silently acquires every cost-bearing capability, which is the auto-adoption #428 exists to prevent.
...
| `release_provenance.production_debug_include_unknown` | REPLACE | Project | Core | Per-repository reporting detail: off, a check records no `unknown` state; held in `project_release_provenance` | #1274 |
| `release_provenance.github.repo` | REPLACE | Project | Core | Names one owner/repo; held in `project_release_provenance`, suggested from the connected repository but never stored silently | #1274 |
| `release_provenance.github.token_env` | REPLACE | Project | Core | Names one repository's credential: now a sealed per-Project slot `release-provenance:{project}:{revision}` used only for release lookups, never the repository read credential; a surface shows only configured or absent and its purpose | #1274 |
...
| `maintenance.auto_optimize` | REPLACE | Deployment | Core | Turns `database-optimize` on. No default: unset is not configured and schedules nothing, and a value the dashboard could not have written is invalid. A Deployment that saved it on with an interval before #1276 runs the check at its first clock wake after upgrading | #1276 |
...
| `cortex.canopy.enabled` | REPLACE | Project | Core | Capability master gate; per-Project admission for the map run, fail-closed when absent. | #1378 |
| `cortex.canopy.refresh.background_enabled` | REPLACE | Deployment | Core | Deployment-side map refresh policy: switches the clock's `canopy-map` schedule on; unset, it is off. | #1378 |
| `cortex.canopy.refresh.background_period_minutes` | REPLACE | Deployment | Core | Deployment-side map refresh schedule: the interval of the clock's `canopy-map` schedule; unset, the declared six hours. | #1378 |
| `cortex.canopy.exclude.default_patterns` | REPLACE | Deployment | Core | Map exclusion the prompt names and the map input hashes. | #1378 |
| `cortex.canopy.exclude.patterns` | REPLACE | Deployment | Core | Map exclusion the prompt names and the map input hashes. | #1378 |
...
| `retention.transcripts` | Records | Days processed raw transcript bytes are kept; unset or 0 keeps them forever, and nothing derived is ever removed |
...
**Leaves whose page is gone are not leaves that are gone.** #1162 drops the Cortex and Skills **pages** (§7.2). Their leaves keep their rows and their controls: `cortex.digest.tier` still sizes the artifact a scheduled run writes, and `skills.confidence_threshold` and `skills.usage_stale_days` are still held by the server. Each is already marked **Planned DROP in #1170 (sweep)** above, and a leaf's disposition changes in the pull request that removes the code reading it, never in one that only removes a page. `cortex.digest.inject_on_session_start` is the one leaf the dashboard shows and refuses to offer: the control is disabled, because a switch that changes nothing reads worse than one that cannot be thrown.

**Project-tier `release_provenance.*` lives in `project_release_provenance`, one row per Project, written only through `core/release-provenance.ts`.** `setLeaf` still refuses these leaves: they are not Deployment settings, and the Project Settings page writes them as one revisioned `PUT /api/projects/{projectId}/release-provenance`, refusing a stale revision. The GitHub credential is sealed in its own slot and read only by the release check. A session's release state is classified from its latest captured commit — the end commit once captured; `unknown` while only the start commit is known or when tracked files differed from the end commit — and a released state changes only for a newer captured commit, keeping the state it replaced.

**Rollout order: the Deployment before its members.** Capture carries the session's commit as `headSha` on `session.start` and `session.end`, and `dirty` on `session.end`; an end whose member could not read `dirty` omits it, is recorded with `error = 'status_unreadable'`, and its session's release state is `unknown`. A Deployment older than schema v46 refuses those payloads as `unknown_field`, so a Deployment is upgraded before the members that capture into it.

**What a record's shape may change is part of the member protocol.** A field outside its declared shape or bound is refused `invalid_field`, which a member treats as final for that record and drops; an unknown field or kind is `unknown_field`/`unknown_kind`, which a member holds for a newer Deployment. Widening an enum (a channel, a prompt origin, a plan status or source, a transcript role), adding a required field, or changing an exactly-one or at-most-one pair therefore changes what one side drops from the other, and ships as a member protocol bump, never inside one. `tests/member/protocol-pins.test.ts` pins those judgements from the worker's catalogue per protocol, so a change to any of them without a new protocol row fails there. The enum lists are one source (`@goondocks/myco-shared/member-protocol`): the worker's catalogue admits them and the member's emitted prompt origin, plan status and transcript role are typed from them, so a member cannot emit a value its own protocol's Deployment refuses.

One further block is **not settled here**, and is marked so rather than assigned silently. `backup.*` describes Deployment backup policy but leads with a filesystem path that has no meaning on a Worker; #923 owns backup/restore and owns the per-target mechanism with it. `maintenance.*` has a mechanism on each target, recorded in its rows and in the `database-optimize` and `database-integrity-check` jobs (#1276).
...
4. **Workers.** A worker on a laptop and a worker on a VM each complete the four run outcomes with close evidence, on at least two harnesses.
...
7. **After the sweep** (plan §5): `packages/myco/src/agent/` is gone, the capture surface is under 4,000 lines, no daemon, Compose, Containers, digest, per-file Canopy, Team-Host or skill-lifecycle code remains, and the ledger gate in §9 passes.

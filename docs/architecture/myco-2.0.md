...
**Planned additions.** Laptop mode's first member is the one join `myco login` cannot serve: a Deployment created by `myco server create --target local` holds no member until an invite can be minted, and the start path exposes its `ServerEnv` for the first-start bootstrap that mints one (**#1158**, after D1). One verb lands with its code and takes a row then: `myco import` (the repeatable backfill behind the join-time pass — newest 50 sessions per harness within 30 days, content-hash dedupe, tombstone gate, plan §2.2, **#1148**).

...
**The member MCP entry is the Deployment's remote HTTP `/mcp` wherever the host can refresh a rotating credential** (#1237). A manifest declaring `memberMcpHeadersHelperKey` gets `url = <registry serverUrl>/mcp` plus a headers helper, `myco member mcp-headers --credential registry --server <serverUrl>`, and no token in the file. Codex (`http_headers_helper`, `.codex/config.toml`) and Claude Code (`headersHelper`, `.mcp.json`) run the helper in the session's directory when they connect. Codex runs it again after any 401, and Claude Code after a 401 or 403 on a tool call, so a rotated member token reaches an open session. The helper reads the registry, never rotates, and prints nothing unless the membership it resolves is on exactly the Deployment `--server` names (`deploymentUrl`), so a stale entry cannot hand one Deployment's bearer to another. Codex deep-merges a same-named global server into the project's and then refuses its whole configuration, so provisioning refuses, before any write, a global `[mcp_servers.myco]` with a stdio transport or a credential of its own (`command`, `args`, `env`, `env_vars`, `cwd`, `bearer_token`, `bearer_token_env_var`, `http_headers`, `env_http_headers`, `oauth`), or a global file it cannot read or parse; it names the file (`MemberMcpConflictError`). A remote global entry with only options such as `startup_timeout_sec` merges and is accepted. **Cursor keeps the stdio member bridge, an explicitly unresolved limitation:** its remote entries take static headers or OAuth and ignore a header helper. Static headers cannot follow a rotating member bearer, and the Deployment serves no MCP OAuth, so Cursor has no compatible refresh path (#1337).

**OpenCode and Pi members get a project member plugin** (#1244). Both hosts load the user-global plugin and the project plugin side by side, and neither can disable a global plugin per project. OpenCode loads every `.opencode/` from the launch directory up to the git root; Pi loads a trusted project's `.pi/extensions` before the global ones. A tool name registered twice while extensions load stops Pi, but the member extension registers its tools at session start, so a global and a member Myco extension would both run and capture every session twice. The global plugin slot belongs to the claim holder (`symbiont-config`), today the retained 1.4 install. So `myco member provision opencode|pi` writes the 2.0 member template to `memberHooksTarget` (`.opencode/plugins/myco.ts`, `.pi/extensions/myco/index.ts`), git-excluded, carrying the line `// myco:member-plugin`, and the retained 1.4 plugin steps aside for it:
...
- **OpenCode's Myco tools come from the project.** `member provision opencode` writes `opencode.json` `mcp.myco` as the stdio bridge `<member-home binary> mcp --credential registry`, the same shape Cursor keeps: OpenCode 1.18.21 has no header-helper field, static headers cannot follow a rotating member token, and the Deployment serves no MCP OAuth, so direct HTTP stays out of reach for this host (#1337 tracks the same limit for Cursor).

...
If the original sign-in credentials are unavailable, add `--new-signin` for either native or hosted recovery. This explicit choice requires only `SECRET_WRAP_KEY` in the independent file. It generates a new destination session secret and omits the old GitHub application credentials, even if they are present in the file. The original wrapping key is still required to open saved encrypted settings. Source sign-in and source credentials are unchanged. The destination's owner sign-in remains unavailable until you run `myco server github-app` for its URL using the same recovery `MYCO_HOME`; start a recovered native server first. A hosted retry must use the same sign-in choice as its saved journal. Dashboard/OAuth acceptance remains pending until that setup succeeds.

...
Hosted lifecycle, record and staging mutations share one operator lease. `server/cloudflare/recovery.json` records the source fingerprint and confirmed resources without secret values. An interrupted data import or object transfer can resume against those same resources. A resource-creation or final-publication call whose result is unknown leaves a named pending operation and refuses automatic retry. Reconcile that specific replacement resource with the provider before changing the journal; do not delete or adopt source resources. There is no automated cleanup of partially provisioned resources. A published destination refuses another restore.

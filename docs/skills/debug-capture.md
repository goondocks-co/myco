...
Top-down procedure for "why didn't this session/prompt/tool-use land in Myco?" Walks the layers from `docs/architecture/capture-lifecycle.md` in order. The first layer whose check fails is the layer the bug lives in. Stop investigating once the failing layer is identified; do not assume two layers are simultaneously broken.

This skill exists because three independent capture regressions (PRs #278, #284, #285, #286) all looked identical from above — "session went silent" — and were each investigated from scratch. The point of the procedure is that the next investigation starts at step 1, not from intuition.
...
- The **agent's transcript path**, if available. The hook payload at Stop carries this; otherwise infer per `docs/architecture/symbiont-capture-contract.md`.
...
(Added by issue #288; in if you're on a daemon version ≥ 0.27.)
...
- `docs/architecture/capture-lifecycle.md` — the layered tenet
- `docs/architecture/symbiont-capture-contract.md` — per-agent capture differences
- `packages/myco/src/daemon/session-lifecycle.ts` — the `ensureSession*` contract

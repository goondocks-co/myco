...
| Published OKF wiki (`<output_path>/`, e.g. `okf/`) + `<projectRoot>/.myco/okf/` state | `OkfBundle` (`packages/myco/src/okf/bundle.ts`) — single writer for the staged-generation transaction (lock + atomic replace), the `state/{manifest,plan,ownership}.json` control state, and publish-eligibility gating. |

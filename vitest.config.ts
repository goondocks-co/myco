...
// Worker-integration tests import worker source (which pulls in `agents/mcp`
// and `cloudflare:workers`) and need a real workerd runtime. Route them to
// the Cloudflare Workers pool via the `cloudflareTest` plugin; everything
// else runs in the default Node pool.
const WORKER_TEST_INCLUDES = ['tests/worker/integration/**/*.test.ts'];
...
const nodeInclude = testProfile === TEST_PROFILE_INTEGRATION ? INTEGRATION_TEST_INCLUDES : DEFAULT_TEST_INCLUDES;
const nodeExclude = [
  ...(testProfile === TEST_PROFILE_FAST ? FAST_TEST_EXCLUDES : DEFAULT_TEST_EXCLUDES),
  ...WORKER_TEST_INCLUDES,
];
        ...
        plugins: [
          cloudflareTest({
            miniflare: {
              ...
              d1Databases: ['MYCO_TEAM_DB'],
              kvNamespaces: ['MYCO_SECRETS'],
            },
          }),
        ],
        test: {
          name: 'worker-integration',
          include: WORKER_TEST_INCLUDES,
        },
      },
    ],
  },

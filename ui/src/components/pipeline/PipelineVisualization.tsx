...
function StageBox({
  ...
  const poisoned = counts?.poisoned ?? 0;
  const totalActive = pending + processing;
  const totalErrors = failed + blocked + poisoned;
      ...
      <div className="flex items-center gap-2 text-xs">
        ...
        {processing > 0 && (
          <span
            ...
            onClick={(e) => { e.stopPropagation(); onClick?.(stage, 'processing'); }}
          >
            {processing} active
          </span>
        )}
        ...
        {poisoned > 0 && (
          <span
            className="font-mono text-red-600/70 dark:text-red-400/70 underline decoration-dotted cursor-pointer"
            onClick={(e) => { e.stopPropagation(); onClick?.(stage, 'poisoned'); }}
          >
            {poisoned} poisoned
          </span>
        )}
        {blocked > 0 && (
          <span
            className="font-mono text-red-600/70 dark:text-red-400/70 underline decoration-dotted cursor-pointer"
            onClick={(e) => { e.stopPropagation(); onClick?.(stage, 'blocked'); }}
          >
            {blocked} blocked
          </span>
        )}
        {succeeded === 0 && totalActive === 0 && totalErrors === 0 && (

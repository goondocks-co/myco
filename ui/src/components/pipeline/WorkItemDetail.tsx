...
function StageOutput({ stage, output }: { stage: string; output: string | null }) {
    ...
    <div className="mt-2 rounded-md border border-border bg-muted/30 px-3 py-2 text-xs space-y-1">
        ...
        <>
          {parsed.observations !== undefined && (
            <div>
              ...
              <span className="font-medium">{String(parsed.observations)} extracted</span>
            </div>
          )}
          {parsed.title && (
            <div>
              <span className="text-muted-foreground">Title:</span>{' '}
              <span className="font-medium">{String(parsed.title)}</span>
            </div>
          )}
          {parsed.summary_tokens !== undefined && (
            <div>
              <span className="text-muted-foreground">Summary:</span>{' '}
              {String(parsed.summary_tokens)} tokens via {String(parsed.model ?? 'unknown')}
            </div>
          )}
        </>
      )}
      {stage === 'embedding' && (
        <div>
          ...
          {String(parsed.dimensions ?? '?')} dimensions via {String(parsed.model ?? 'unknown')}
        </div>
      )}
      {stage === 'consolidation' && (
        <>
          {Array.isArray(parsed.superseded) && parsed.superseded.length > 0 ? (
            <div>
              <span className="text-muted-foreground">Superseded:</span>{' '}
              {String(parsed.superseded.length)} older spores
            </div>
          ) : (
            <div className="text-muted-foreground">No similar spores found</div>
          )}
          {parsed.candidates_evaluated !== undefined && Number(parsed.candidates_evaluated) > 0 && (
            <div>
              <span className="text-muted-foreground">Candidates evaluated:</span>{' '}
              {String(parsed.candidates_evaluated)}
            </div>
          )}
        </>
      )}
      {stage === 'digest' && !!parsed.included_in_cycle && (
        <div>
          <span className="text-muted-foreground">Included in cycle</span>{' '}
          <span className="font-mono">{String(parsed.included_in_cycle).slice(0, 8)}</span>
          {Array.isArray(parsed.tiers_generated) && (
            <span> — {String(parsed.tiers_generated.length)} tiers generated</span>
          )}
        </div>
      )}
    </div>
...
function TimelineNode({
  ...
  const isErrorState = stageStatus.status === 'poisoned' || stageStatus.status === 'failed';
  ...
  const handleSkip = async () => {
    setSkipping(true);
    try {
      await postJson(`/pipeline/skip/${encodeURIComponent(itemId)}`, {
        type: itemType,
        stage: stageStatus.stage,
      });
      onRetried();
    ...
    <div className="flex gap-3">
      ...
      <div className="flex-1 pb-4">
        ...
        {stageStatus.status === 'succeeded' && (
          <StageOutput stage={stageStatus.stage} output={stageStatus.output} />
        )}
      </div>
    </div>
  );
...
function ReprocessButton({ sessionId, onDone }: { sessionId: string; onDone: () => void }) {
  const [running, setRunning] = useState(false);

  const handleReprocess = async () => {
    setRunning(true);
    try {
      await postJson('/reprocess', { session: sessionId });
      onDone();
    } finally {
      setRunning(false);
    }
  ...
  return (
    <div className="space-y-3 rounded-md border border-border bg-card/50 px-4 py-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {data.type === 'session' && (
          <ReprocessButton sessionId={data.id} onDone={() => refetch()} />
        )}
      </div>

...
describe('housekeeping on the Operations page', () => {
  ...
  it('words the backfill in every state, and its policy', () => {
    ...
    expect(progressWords({ ...base, owed: 4, remaining: 1, runsPerDay: null, runIn: ['idle'], intervalSeconds: 60 })).toBe('4 ended live sessions waiting for a title; these are titled automatically. 1 fully parsed imported session waiting for a title attempt. Dispatches while the server is idle, at most once every 1 min. Today: 3 started, 1 in flight, 2 titled, 0 failed.');
    expect(progressWords({ ...base, scheduledTasksEnabled: false, enabled: false })).toBe('No ended live sessions are waiting for a title. No fully parsed imported sessions are waiting for a title attempt. The imported-session backfill is on but runs only while scheduled intelligence is on; turn that on in Settings. Dispatches while the server is in use or idle, at most once every 15 min. Today: 3 of 24 started, 1 in flight, 2 titled, 0 failed.');
    expect(progressWords({ ...base, backfillEnabled: false, enabled: false, remaining: 2, owed: 1 })).toBe('1 ended live session waiting for a title; these are titled automatically. 2 fully parsed imported sessions waiting for a title attempt. The imported-session backfill is stopped. Dispatches while the server is in use or idle, at most once every 15 min. Today: 3 of 24 started, 1 in flight, 2 titled, 0 failed.');

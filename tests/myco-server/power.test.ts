...
describe('what runs at each depth', () => {
   ...
   *
   * Work an inactive Deployment still owes its owner is held shallower instead:
   * a due backup asserts `recovery:due`, which caps the state at sleep.
   */
  ...
  it('still runs query-only housekeeping while sleeping: every job declared to run through sleep, and no other', () => {
    ...
    expect(due).toEqual(expect.arrayContaining(['agent-run-retention', 'run-stale-sweep', 'object-release-drain', SCHEDULE_JOB]));
  });

...
describe('route table', () => {
   ...
   */
  const QUOTA_EXEMPT = new Set(['/tokens/refresh', '/mcp', '/members/link-github', '/runs/admission', '/runs/claim', '/runs/get', '/runs/update', '/runs/failed', '/runs/resume-admission', '/runs/supersede', '/runs/reports', '/runs/report', '/runs/events', '/runs/session-material', '/runs/session-title', '/runs/spores', '/runs/spore', '/runs/spore-create', '/runs/spore-resolve', '/runs/instruction', '/runs/instructions-write', '/runs/sessions', '/runs/digest', '/runs/digest-write', '/runs/state/read', '/runs/state/write', '/spores/save', '/spores/list', '/spores/get', '/spores/resolve', '/context/prompt', '/context/session']);
  ...
  it('declares an auth kind and a body mode for every route, a shape for every member route, and charges every member route to the quota but the named exemptions', () => {
    ...
    expect(ROUTES.filter((r) => r.auth === 'public' || r.auth === 'member').map((r) => `${r.method} ${r.path}`)).toEqual(['GET /health', 'POST /events', 'POST /blobs/{sha256}', 'POST /tokens/refresh', 'POST /runs/claim', 'POST /runs/admission', 'POST /runs/get', 'POST /runs/update', 'POST /runs/failed', 'POST /runs/resume-admission', 'POST /runs/supersede', 'POST /runs/reports', 'POST /runs/report', 'POST /runs/events', 'POST /runs/session-material', 'POST /runs/session-title', 'POST /runs/spores', 'POST /runs/spore', 'POST /runs/spore-create', 'POST /runs/spore-resolve', 'POST /runs/instruction', 'POST /runs/instructions-write', 'POST /runs/sessions', 'POST /runs/digest', 'POST /runs/digest-write', 'POST /spores/save', 'POST /spores/list', 'POST /spores/get', 'POST /spores/resolve', 'POST /context/prompt', 'POST /context/session', 'POST /runs/state/read', 'POST /runs/state/write', 'POST /mcp', 'POST /members/link-github']);
  });

...
describe('server schema', () => {
  ...
  it('adds producer identity, the spill key, and payload bytes to events, and only facts to sessions', () => {
    ...
    expect(sessions).toEqual(['project_id', 'session_id', 'machine_id', 'created_by_token_id', 'first_received_at', 'last_received_at', 'agent', 'branch', 'started_at', 'ended_at', 'origin_path', 'parent_session_id', 'parent_reason', 'facts_event_id', 'title', 'summary', 'titled_at', 'titled_by', 'titling_requested_at']);

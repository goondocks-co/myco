...
describe('PipelineManager', () => {
  ...
  describe('stage output tracking', () => {
    it('stage_transitions table has output column', () => {
      const columns = manager.getDb().pragma('table_info(stage_transitions)') as Array<{ name: string }>;
      expect(columns.map((c) => c.name)).toContain('output');
    });

    it('output column defaults to null', () => {
      manager.register('test-1', 'session', 'sessions/test.md');
      manager.advance('test-1', 'session', 'capture', 'succeeded');
      const row = manager.getDb()
        .prepare('SELECT output FROM stage_transitions WHERE work_item_id = ? ORDER BY id DESC LIMIT 1')
        .get('test-1') as { output: string | null };
      expect(row.output).toBeNull();
    });

    it('stores output JSON when status is succeeded', () => {
      manager.register('out-1', 'session', 'sessions/test.md');
      const output = { observations: 3, title: 'Test Session' };
      manager.advance('out-1', 'session', 'capture', 'succeeded', undefined, output);
      const row = manager.getDb()
        .prepare('SELECT output FROM stage_transitions WHERE work_item_id = ? AND stage = ? ORDER BY id DESC LIMIT 1')
        .get('out-1', 'capture') as { output: string | null };
      expect(row.output).toBe(JSON.stringify(output));
    });

    it('discards output when status is not succeeded', () => {
      manager.register('out-2', 'session', 'sessions/test.md');
      manager.advance('out-2', 'session', 'capture', 'processing', undefined, { observations: 3 });
      const row = manager.getDb()
        .prepare('SELECT output FROM stage_transitions WHERE work_item_id = ? AND stage = ? ORDER BY id DESC LIMIT 1')
        .get('out-2', 'capture') as { output: string | null };
      expect(row.output).toBeNull();
    });

    it('discards output on failed status even if provided', () => {
      manager.register('out-3', 'session', 'sessions/test.md');
      manager.advance('out-3', 'session', 'capture', 'failed',
        { errorType: 'transient', errorMessage: 'timeout' },
        { some: 'data' });
      const row = manager.getDb()
        .prepare('SELECT output FROM stage_transitions WHERE work_item_id = ? AND stage = ? ORDER BY id DESC LIMIT 1')
        .get('out-3', 'capture') as { output: string | null };
      expect(row.output).toBeNull();
    });
  });

  describe('compaction with output', () => {
    it('copies last succeeded output to stage_history', () => {
      manager.register('comp-1', 'session', 'sessions/test.md');
      const oldDate = new Date(Date.now() - 60 * MS_PER_DAY).toISOString();
      const db = manager.getDb();
      db.prepare(
        `INSERT INTO stage_transitions (work_item_id, item_type, stage, status, attempt, output, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
      ).run('comp-1', 'session', 'capture', 'processing', 1, null, oldDate);
      db.prepare(
        `INSERT INTO stage_transitions (work_item_id, item_type, stage, status, attempt, output, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
      ).run('comp-1', 'session', 'capture', 'succeeded', 1, JSON.stringify({ observations: 5 }), oldDate);
      const result = manager.compact(1);
      expect(result.compacted).toBeGreaterThan(0);
      const history = db.prepare(
        'SELECT last_output FROM stage_history WHERE work_item_id = ? AND stage = ?',
      ).get('comp-1', 'capture') as { last_output: string | null };
      expect(history.last_output).toBe(JSON.stringify({ observations: 5 }));
    });

    it('sets last_output to null when no succeeded transition in group', () => {
      manager.register('comp-2', 'session', 'sessions/test.md');
      const oldDate = new Date(Date.now() - 60 * MS_PER_DAY).toISOString();
      const db = manager.getDb();
      db.prepare(
        `INSERT INTO stage_transitions (work_item_id, item_type, stage, status, attempt, output, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
      ).run('comp-2', 'session', 'capture', 'failed', 1, null, oldDate);
      const result = manager.compact(1);
      expect(result.compacted).toBeGreaterThan(0);
      const history = db.prepare(
        'SELECT last_output FROM stage_history WHERE work_item_id = ? AND stage = ?',
      ).get('comp-2', 'capture') as { last_output: string | null };
      expect(history.last_output).toBeNull();
    });
  });

  describe('output in status queries', () => {
    it('getItemStatus includes output field', () => {
      manager.register('q-1', 'session', 'sessions/test.md');
      manager.advance('q-1', 'session', 'capture', 'succeeded', undefined, { observations: 3 });
      const stages = manager.getItemStatus('q-1', 'session');
      const capture = stages.find((s) => s.stage === 'capture');
      expect(capture?.output).toBe(JSON.stringify({ observations: 3 }));
    });

    it('getTransitionHistory includes output field', () => {
      manager.register('q-2', 'session', 'sessions/test.md');
      manager.advance('q-2', 'session', 'capture', 'succeeded', undefined, { title: 'Test' });
      const history = manager.getTransitionHistory('q-2', 'session');
      const succeeded = history.find((t) => t.status === 'succeeded');
      expect(succeeded?.output).toBe(JSON.stringify({ title: 'Test' }));
    });
  });

  describe('advanceDigestItems with output', () => {
    it('passes output to advance for each digest item', () => {
      manager.register('dig-1', 'session', 'sessions/test.md');
      manager.advance('dig-1', 'session', 'capture', 'succeeded');
      manager.advance('dig-1', 'session', 'extraction', 'succeeded');
      manager.advance('dig-1', 'session', 'embedding', 'succeeded');

      const output = { included_in_cycle: 'cycle-123', tiers_generated: [1500, 3000] };
      const count = manager.advanceDigestItems(output);
      expect(count).toBe(1);

      const stages = manager.getItemStatus('dig-1', 'session');
      const digest = stages.find((s) => s.stage === 'digest');
      expect(digest?.output).toBe(JSON.stringify(output));
    });
  });

  describe('newSubstrateSinceLastDigest', () => {
    it('counts items at digest:pending where all other stages are terminal', () => {
      manager.register('sub-1', 'session', 'sessions/test.md');
      manager.advance('sub-1', 'session', 'capture', 'succeeded');
      manager.advance('sub-1', 'session', 'extraction', 'succeeded');
      manager.advance('sub-1', 'session', 'embedding', 'succeeded');
      const count = manager.newSubstrateSinceLastDigest();
      expect(count).toBe(1);
    });

    it('does not count items with non-terminal upstream stages', () => {
      manager.register('sub-2', 'session', 'sessions/test.md');
      manager.advance('sub-2', 'session', 'capture', 'succeeded');
      const count = manager.newSubstrateSinceLastDigest();
      expect(count).toBe(0);
    });

    it('counts items with skipped stages as ready', () => {
      manager.register('sub-3', 'artifact', 'artifacts/test.md');
      manager.advance('sub-3', 'artifact', 'capture', 'succeeded');
      manager.advance('sub-3', 'artifact', 'embedding', 'succeeded');
      const count = manager.newSubstrateSinceLastDigest();
      expect(count).toBe(1);
    });

    it('does not count items already at digest:succeeded', () => {
      manager.register('sub-4', 'session', 'sessions/test.md');
      manager.advance('sub-4', 'session', 'capture', 'succeeded');
      manager.advance('sub-4', 'session', 'extraction', 'succeeded');
      manager.advance('sub-4', 'session', 'embedding', 'succeeded');
      manager.advance('sub-4', 'session', 'digest', 'succeeded');
      const count = manager.newSubstrateSinceLastDigest();
      expect(count).toBe(0);
    });
  });

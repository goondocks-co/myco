...
const TEST_CURATOR_ID = 'curator-tasks-test';
...
describe('task query helpers', () => {
  ...
  describe('listTasks', () => {
    ...
    it('filters by curator_id', async () => {
      ...
      await upsertTask(makeTask({ id: 'task-b', curator_id: 'curator-other' }));

      const rows = await listTasks({ curator_id: TEST_CURATOR_ID });
  ...
  describe('getDefaultTask', () => {
    it('returns the default task for a curator', async () => {
      ...
      const defaultTask = await getDefaultTask(TEST_CURATOR_ID);
    ...
    it('returns null for curator with no tasks', async () => {
      const defaultTask = await getDefaultTask('no-such-curator');
      expect(defaultTask).toBeNull();
    });
  });
});

...
/** The one admission for the one counter. What the token has been charged (`member_tokens.bytes_written`: its stored event body bytes plus its stored blob bytes — the quantity the quota CHECK enforces), plus what its live reservations hold (all but `except`, a reservation the caller is re-sizing), plus `bytes` stays inside the quota. Every writer of the counter — the event raw insert, the blob reservation, and the reservation reconcile — admits through this fragment and no other. */
export function withinQuota(ctx: QuotaContext, bytes: number, except: string | null = null): Fragment {
  return {
    ...
    params: [ctx.tokenId, ctx.projectId, ctx.tokenId, ctx.now, ...(except === null ? [] : [except]), bytes],
  };
}

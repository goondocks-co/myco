...
| T5 | `sql "UPDATE member_tokens SET lineage_started_at = expires_at - 7776000000 WHERE id = '<SID2>'"`, `expires_at` moved to now + 1 h, then `refresh "$S2"` | `{refreshed:false, code:'lineage_expired', reason:'token lineage expired'}` |
| T6 | `refresh "$S2"` with body `not json` | `{refreshed:false, code:'parse', reason:'body must be JSON'}` |
| T7 | `sql "$(npm run -s token:revoke -- <SID2> --lineage)"` | reports 1 row changed (`<SID2>` was the chain's only live token); `post "$S2"` → `401`; `post "$T2"` (another lineage) unaffected |

...
export async function handleRevokeToken(env: ServerEnv, ctx: OwnerContext): Promise<Response> {
  const scope = await resolveProjectScope(env.db, ctx.member, ctx.params.projectId);
  if (scope === null) return notFound();
  // The revoking member, by id: the actor form a runtime's own revocation records, on either channel.
  return ok(await revokeCredentialAsMember(env.db, ctx.member.id, ctx.params.tokenId, ctx.now));
}

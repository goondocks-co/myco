...
import { ATTACHMENT_QUERY, RESPONSE_QUERY, listChildren, type AttachmentRow, type ResponseRow } from './children.js';
...
export async function turnDetail(db: RelationalStore, scope: ReadScope, sessionId: string, promptId: string): Promise<TurnDetail | null> {
  ...
  const [responses, attachments, childRows] = await Promise.all([
  ...
  return { prompt, responses: [...responses.rows], attachments: [...attachments.rows], children };
}

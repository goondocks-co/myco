import { InvalidSearch, searchProject, SEARCH_TOOL_LIMIT, type SearchOptions } from '../../read/search.js';
...
export async function handleSearch(input: ToolInput, ctx: ToolContext): Promise<unknown> {
  ...
  for (const key of ['type', 'status', 'observation_type', 'release_state', 'release_confidence'] as const) {
  ...
  try { return await searchProject(ctx.env.db, scope, options); }
  catch (error) {

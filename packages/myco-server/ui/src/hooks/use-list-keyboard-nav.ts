...
export interface UseListKeyboardNavOptions<T> {
  ...
  /** The list's filter box; `/` focuses it. */
...
/** j/k and the arrow keys move a cursor over a rail's rows, Enter opens the row under it, `/` jumps to the filter; the cursor follows the selection when it changes from outside. */
export function useListKeyboardNav<T>({ items, getId, selectedId, onActivate, filterInputRef, enabled = true }: UseListKeyboardNavOptions<T>): UseListKeyboardNavResult {
  ...
  useEffect(() => {
    if (cursorIndex >= items.length) setCursorIndex(Math.max(0, items.length - 1));
  ...
  return { cursorIndex, setRowRef, containerProps: { tabIndex: 0, onKeyDown } };
}

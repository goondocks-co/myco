...
export function promptPreview(turn: Pick<TurnRow, 'preview' | 'textChars' | 'blobKey'>): string {
  if (turn.preview === null) return turn.blobKey !== null ? 'Stored text' : '(no prompt)';
        ...
        <div key={r.responseId} className="flex gap-3 px-4 py-3" data-testid="turn-response">
          ...
          <div className="min-w-0 flex-1">
            <div className="mb-1.5 flex items-center gap-1.5">
              ...
              <span className="ml-auto font-mono text-[10px] text-on-surface-variant">{formatRelative(r.createdAt)}</span>
            </div>
  ...
  return (
    <div className="mx-4 mt-3 border-l-2 border-primary/30 pb-3 pl-4" data-testid="turn-child">
      ...
      <div className="mb-1 flex items-baseline justify-between gap-2">
        ...
        <span className="shrink-0 font-mono text-xs text-on-surface-variant">{formatDateTime(child.prompt.createdAt)}</span>
      </div>
...
export function TurnCard({ projectId, sessionId, turn, index, isLast, defaultOpen = false }: TurnCardProps) {
  ...
  return (
    <div className="relative flex gap-2" data-testid={`turn-${turn.promptId}`} data-origin={turn.origin} style={open ? undefined : { contentVisibility: 'auto', containIntrinsicSize: 'auto 72px' }}>
      ...
      <Surface level="low" className={cn('mb-2 max-w-full flex-1 overflow-hidden rounded-md border border-outline-variant/10', system && 'opacity-80')}>
        <button
          type="button"
          aria-expanded={open}
          onClick={() => setOpen((v) => !v)}
        ...
        >
          ...
          <div className="min-w-0 flex-1 overflow-hidden">
            <div className="mb-0.5 flex items-baseline justify-between gap-2">
              <span className={cn(eyebrow, 'shrink-0')}>{system ? turn.origin.replace('_', ' ') : 'Prompt'}</span>
            ...
            <p className="truncate font-sans text-sm text-on-surface">{promptPreview(turn)}</p>
          </div>
        </button>

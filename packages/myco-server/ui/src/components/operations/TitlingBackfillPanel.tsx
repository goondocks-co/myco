...
/** Where titling stands, in the reader's words: sessions captured live converge on their own; imported sessions wait for the backfill switch. */
export function progressWords(p: BackfillProgress): string {
  const owed = p.owed === 0 ? 'No ended live sessions are waiting for a title.' : `${plural(p.owed, 'ended live session')} waiting for a title; these are titled automatically.`;
...
/** Title every ended session a few a day, newest first: live sessions on their own, imported ones while the backfill is on; stop or resume that backfill here. */

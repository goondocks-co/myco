...
export function Measures() {
  ...
  const window = isWindow(params.get('window')) ? params.get('window') as MeasureWindow : DEFAULT_MEASURE_WINDOW;
  const kpis = useKpis(window);
      ...
      <PageLoading isLoading={kpis.isPending} error={kpis.error}>
        {report && (
          <div className="flex flex-col gap-6">
            ...
            <Panel eyebrow="Diagnostic" title="Myco calls per prompt, by agent" padded={report.callsPerPromptByHarness.length === 0}>
              {report.callsPerPromptByHarness.length === 0 ? (
                ...
                <table className="w-full font-sans text-sm">
                  <thead className="text-left font-mono text-[10px] uppercase tracking-wide text-outline">
                    <tr><th className="py-1">Agent</th><th className="py-1">Calls per prompt</th><th className="py-1">Sample</th></tr>
                  </thead>
                  <tbody aria-label="Calls per prompt by agent">
                    {report.callsPerPromptByHarness.map((row) => (
                      <tr key={row.harness} className="border-t border-[var(--ghost-border)]">
                        ...
                        <td className="py-1.5 font-mono text-[11px] text-on-surface-variant">n = {row.sampleSize.toLocaleString()} prompts</td>
                      </tr>
                    ))}
                  </tbody>
                </table>

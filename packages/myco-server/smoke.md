...
| T5 | `sql "UPDATE member_credentials SET issued_at = <now> - 7776000000, expires_at = <now> - 7171200000 WHERE id = '<SID2>'"` (one statement: the token lapsed, and its lineage has not refreshed for 90 days), then `refresh "$S2"` | `{refreshed:false, code:'lineage_expired', reason:'token lineage inactive'}`; no new row. Before #1417 this step moved `lineage_started_at` to the lineage's 90-day ceiling instead; a lineage no longer ends for age |

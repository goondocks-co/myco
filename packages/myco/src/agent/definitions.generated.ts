...
export const BUNDLED_AGENT_TASKS: readonly AgentTask[] = [
  ...
  },
  {
    ...
    "phases": [
      ...
      {
        "name": "digest-10000",
        "prompt": "## STOP-FIRST CHECK (turn 1 — do this before anything else)\n\nDefault to SKIP. Only proceed past this section if you find\nAFFIRMATIVE evidence in the prior `digest-assess` phase summary\nthat tier 10000 was selected this run.\n\nAffirmative evidence means BOTH of:\n- The summary contains the literal string `Chosen tier for this run: 10000`\n  (digit 1-0-0-0-0, not another tier).\n- AND the per-tier directives section lists `10000: UPDATE` (not\n  `10000: SKIP`).\n\nIf you cannot find both of those, you MUST stop now. This includes:\n- digest-assess reported a skip-shape (e.g. \"all tiers fresh\",\n  \"no rotation\", \"no new intelligence material this run\") and never\n  emitted per-tier directives.\n- digest-assess selected a different tier (5000 or 1500).\n- The summary is ambiguous and you cannot tell.\n\nOn STOP: do not call `vault_read_digest`. Do not call\n`vault_search_semantic`. Do not call `vault_write_digest`. Call\n`vault_report` with action \"skip\" and reason \"tier 10000 not selected\nthis rotation\", then finish.\n\n## If you ARE the selected tier\n\nUpdate digest tier 10000 — Full institutional knowledge.\nBudget: 9 turns total (you've already used 1 on the STOP check).\n\nThis is the largest tier (~10,000 tokens). The digest-assess phase\nsummary in your context contains the material to integrate.\n\n1. Call `vault_read_digest` with tier 10000 to get current content\n2. Call `vault_search_semantic` for themes identified in the assess\n   findings — gather specific details to weave in\n3. Integrate new material into the existing digest\n4. Call `vault_write_digest` with the updated content\n\nThe existing digest is your baseline — integrate new material, don't\nrewrite from scratch. Preserve well-crafted existing content.\n\nPrioritize: recent insights > active decisions > unresolved gotchas >\narchitectural patterns > historical context.\n\nFinal summary MUST be ≤ 5 lines.\n",
        "tools": [
          "vault_search_semantic",
          "vault_release_state",
          "vault_read_digest",
      ...
      {
        "name": "digest-5000",
        "prompt": "## STOP-FIRST CHECK (turn 1 — do this before anything else)\n\nDefault to SKIP. Only proceed past this section if you find\nAFFIRMATIVE evidence in the prior `digest-assess` phase summary\nthat tier 5000 was selected this run.\n\nAffirmative evidence means BOTH of:\n- The summary contains the literal string `Chosen tier for this run: 5000`\n  (digit 5-0-0-0, not another tier).\n- AND the per-tier directives section lists `5000: UPDATE` (not\n  `5000: SKIP`).\n\nIf you cannot find both of those, you MUST stop now. This includes:\n- digest-assess reported a skip-shape (e.g. \"all tiers fresh\",\n  \"no rotation\", \"no new intelligence material this run\") and never\n  emitted per-tier directives.\n- digest-assess selected a different tier (10000 or 1500).\n- The summary is ambiguous and you cannot tell.\n\nOn STOP: do not call `vault_read_digest`. Do not call\n`vault_search_semantic`. Do not call `vault_write_digest`. Call\n`vault_report` with action \"skip\" and reason \"tier 5000 not selected\nthis rotation\", then finish.\n\n## If you ARE the selected tier\n\nUpdate digest tier 5000 — Deep onboarding.\nBudget: 7 turns total (you've already used 1 on the STOP check).\n\nThis tier (~5,000 tokens) focuses on trade-offs and patterns. The\ndigest-assess phase summary in your context contains the material\nto integrate.\n\n1. Call `vault_read_digest` with tier 5000 to get current content\n2. Optionally call `vault_search_semantic` for additional context\n3. Integrate new material from the assess findings\n4. Call `vault_write_digest` with the updated content\n\nThe existing digest is your baseline — integrate new material, don't\nrewrite from scratch. Preserve well-crafted existing content.\n\nPrioritize: recent insights > active decisions > unresolved gotchas >\narchitectural patterns > historical context.\n\nFinal summary MUST be ≤ 5 lines.\n",
        "tools": [
          "vault_search_semantic",
          "vault_release_state",
          "vault_read_digest",

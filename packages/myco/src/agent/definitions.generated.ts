  ...
  },
  {
    ...
    "phases": [
      ...
      },
      {
        ...
        "prompt": "Persist the queue management plan from reconcile-queue.\n\nBefore each write, briefly re-check the target with\nvault_skill_candidates or vault_skill_records if needed. The tool\nenforces quality gates, so fix the plan instead of retrying a\nrejected write unchanged.\n\n## Candidate writes\n\nUse vault_skill_candidates (action: create) with:\n- topic: the domain name (e.g., \"Vault Schema and Data Layer Extension\")\n- rationale: markdown with these sections:\n  - **Domain scope**: what procedures this covers (bulleted list)\n  - **Procedure verdict**: PASS/FAIL\n  - **Repeatability verdict**: PASS/FAIL with justification\n  - **Breadth verdict**: PASS/FAIL with section count estimate\n  - **Cross-session evidence**: session IDs and source count\n  - **Existing coverage**: skills/candidates checked and why this\n    is new or broader (name what you checked, not \"NONE\")\n  - **Supersedes**: which existing narrow skills this replaces\n    (if any)\n- confidence: 0.0-1.0\n- evidence_bundle_id: bundle id from the review\n- quality_score: 0.7-1.0\n- quality_failures: [] for identified candidates\n- coverage_matches: JSON array of string identifiers for related\n  skills/candidates/dismissed candidates\n- source_ids: JSON array of {id, type} objects\n  (at least 3 valid refs for identified candidates)\n- supersedes: JSON array of skill names this replaces (or omit)\n\nIf an existing candidate has new evidence, use action: update\nwith the same metadata requirements.\n\nDefer candidates that are plausible but not ready:\naction: update, status: deferred, quality_failures containing the\ncanonical reason codes. Only defer existing queue items; do not\ncreate new deferred candidates from weak bundles.\n\nDismiss candidates that are generic, duplicates, covered by an\nactive skill, or superseded by newer knowledge:\naction: update, status: dismissed, quality_failures containing\nthe canonical reason codes.\n\nDo not update approved or generated candidates. Report any\nstale/duplicate approved or generated candidates in vault_report\nfor human follow-up.\n\nDo not leave duplicate low-quality identified candidates in the\nqueue just because the human may review them later.\n\nReport a summary via vault_report.\n",
        "tools": [
          "vault_skill_candidates",
          "vault_skill_records",
          "vault_set_state",

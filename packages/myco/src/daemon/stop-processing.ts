...
export function createStopProcessor(deps: StopProcessorDeps): {
  ...
  const handleStopRoute: RouteHandler = async (req) => {
    ...
    // CLI therefore buffers every summary-bearing stop, by design: replay
    // is NULL-only idempotent, so a buffered copy of a turn the queue
    // later persists converges as a no-op.

...
const CORTEX_TABS = [
  { id: 'instructions', label: 'Instructions', description: 'session-start' },
  { id: 'builder', label: 'Builder', description: 'prompt forge' },
  { id: 'digest', label: 'Digest', description: 'context tiers' },
  { id: 'canopy', label: 'Canopy', description: 'code index' },
] as const;
...
export default function Cortex() {
  ...
  return (
    <PageContainer>
      ...
      <TileTabs
        tabs={CORTEX_TABS.map((tab) => ({ id: tab.id, label: tab.label, description: tab.description }))}
        activeTab={activeTab}
...
function InstructionsTab() {
  ...
  return (
    <div className="space-y-6">
      <Surface level="low" accent="sage" className="rounded-lg border border-outline-variant p-5">
        <div className="flex flex-wrap items-start gap-3">
          <div className="rounded-md bg-sage/10 p-2 text-sage">
            <Brain className="h-4 w-4" />
          </div>
          <div className="space-y-1">
            <Eyebrow tone="sage">Cortex · instructions</Eyebrow>
            <h3 className="myco-display-sm text-on-surface m-0">
      ...
      <Surface level="low" className="rounded-lg border border-outline-variant/20 p-6 space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="space-y-1">
            ...
            <h3 className="myco-display-sm text-on-surface m-0">Current Instructions</h3>
            <p className="font-sans text-sm text-on-surface-variant">

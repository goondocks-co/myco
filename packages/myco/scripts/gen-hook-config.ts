...
`;
  return { hookConfig, bundledManifests, captureRules, symbiontCount: manifests.length };
}

function main(): void {
  const { hookConfig, bundledManifests, captureRules, symbiontCount } = renderHookConfigSources();

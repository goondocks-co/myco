...
**Bun's test runner is JavaScriptCore; Cloudflare Workers production is V8 — a green `bun test` does not prove engine-safe behavior.** Bun executes tests under JSC while Cloudflare Workers (workerd/miniflare) run V8, and the two engines diverge on stack-depth and spread-argument limits. Confirmed during a security review: a pure-JS bound check (`next.push(...Object.values(v))` in a payload depth walker) passed under `bun test` but threw `RangeError: Maximum call stack size exceeded` under production V8. For code paths that run on Workers, do not treat a passing Bun test as proof of engine-safe recursion/spread behavior — validate stack-sensitive logic against miniflare/workerd, not just Bun's JSC runtime.

**A global `npm install -g bun` can silently shadow the real Bun binary with a ~450-byte postinstall stub.** This has recurred on the same machine: the npm-global stub becomes whatever `bun` resolves to on `PATH`, and every `npm test` invocation then fails with "postinstall script was not run" instead of running tests. `warnOnBunVersionSkew()` (`scripts/run-bun-tests.mjs:136-151`) does not catch this — it checks a Bun-only runtime version global that is unset when the runner is spawned via `node`, so the check never fires under the canonical `npm test` invocation. Proven repair: `npm uninstall -g bun`, then reinstall via the official bun.sh installer pinned to the repo's `.bun-version`.

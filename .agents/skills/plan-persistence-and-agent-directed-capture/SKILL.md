...
**Codex `proposed_plan` vs `update_plan`**: These are distinct Codex signals. `update_plan` is a transient checklist/progress function — Codex documents it as a non-Plan-Mode tool. Durable Plan Mode output is always assistant text wrapped in `<proposed_plan>...</proposed_plan>`. The Codex manifest `planTags` must remain `["proposed_plan"]`. Setting it to `["update_plan"]` causes plan capture to miss all real Codex plans while capturing transient checklist state as false positives.

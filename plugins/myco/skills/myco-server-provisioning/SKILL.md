...
Every verb checks those prerequisites before it runs anything: wrangler resolvable without a fetch (`npx --no-install`), and a Cloudflare identity — an interactive `wrangler login`, or `CLOUDFLARE_API_TOKEN` in a shell that cannot open a browser. Each missing one is refused by name in one line, so the first thing an operator reads is Myco's instruction rather than wrangler's.
